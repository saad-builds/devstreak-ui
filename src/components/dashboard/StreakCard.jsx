...
function getRank(n) {
  ...
  if (n >= 100)
    return { label: "ELITE", color: "#f59e0b", bg: "rgba(245,158,11,0.18)" };
  if (n >= 30)
    return {
      label: "CONSISTENT",
      color: "#f97316",
  ...
  if (n >= 7)
    return { label: "RISING", color: "#38bdf8", bg: "rgba(56,189,248,0.18)" };
...
function drawCardToCanvas(canvas, user, logs, recentLog, scaleMultiplier = 1) {
  ...
  ctx.textAlign = "right";
  ctx.fillText("devstreak-ui.vercel.app", BASE_W - padX, hmY + cellSize + 65);
  ctx.textAlign = "left";
}
...
const StreakCard = forwardRef(({ user, logs = [], recentLog = null }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    download: (filename) => {
      ...
      link.download =
        filename || `devstreak-day-${user?.currentStreak || 0}.png`;
    ...
    <div style={{ width: "100%", maxWidth: "600px", margin: "0 auto" }}>
      <canvas
        ...
        style={{
          ...
          display: "block",
          boxShadow:
            "0 25px 30px -5px rgba(0, 0, 0, 0.5), 0 10px 12px -6px rgba(0, 0, 0, 0.3)",
        }}
      />
    </div>
  );
